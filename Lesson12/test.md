...
### 4 Опишите смысл команд: pass, return, continue, break, yield в языке Python.
    ...
    Pass - оператор-заместитель,полезныйдлясозданиясинтаксическикорректного,ноничегоневыполняющегокода.
    ...
    Yield - используется в функциях-генераторах для созданияповторяющихсяпоследовательностей.
